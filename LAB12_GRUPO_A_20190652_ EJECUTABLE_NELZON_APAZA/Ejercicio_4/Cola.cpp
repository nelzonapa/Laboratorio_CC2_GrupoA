...
void COLA::agregar_elem(int dato_ingresado) {//para agregar un elemnto a la cola
    t=new(Nodo);//separamos espacio
    t->dato=dato_ingresado;//escribimos el dato
...
int COLA::get_dato(int iteraciones){//Para devolver elemento en cierta posición(iteracion)
	int contador=1;//para que encaje
	for(t=q.frente;t!=NULL;t=t->enlace){//avanzamos hasta enocontrar el NULL
