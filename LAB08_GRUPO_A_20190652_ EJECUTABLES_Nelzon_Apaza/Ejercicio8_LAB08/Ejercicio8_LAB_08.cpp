...
int main(){
    ...
    /*-----------------Calculamos e imprimimos el área máxima de todas las formas----------------*/
   float max_area,area=0;
   int aux_posicion;
   max_area=vector_formas[0]->get_area();//ponemos el primer valor en un auxiliar para hacer la comparación
    for (int i = 0; i < 4; i++)
    {
        area=vector_formas[i]->get_area();
        if (max_area==area or max_area<area)
        {
            max_area=area;//almacenamos el area máxima
            aux_posicion=i;
        }
    }
    cout<<endl<<"-----------Esta es la FORMA o FIGURA con area maxima: -----------"<<endl;
    vector_formas[aux_posicion]->imprimir();
    vector_formas[aux_posicion]->area();
