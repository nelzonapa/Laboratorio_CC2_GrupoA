...
void Lista::Eliminar_primero(){
    ...
    Nodo *Extra_cabeza=NULL;
    Nodo *Anterior=NULL;//para mantener la cadena
    Nodo *Avanzar;
    Avanzar=get_cabeza();
    // Avanzar=Avanzar->get_enlace();//avanza una vez
    ...
    //necistamos eliminar el CABEZA
    else{
        Avanzar=Avanzar->get_enlace();//avanza una vez
        set_cabeza(Extra_cabeza);//eliminamos o ponemos NULL a la cabeza
        for (int i=2;i<=numero_nodos;i++)//avanza toda la lista de nodos desde la posición 2, ya no la cabeza
        {
