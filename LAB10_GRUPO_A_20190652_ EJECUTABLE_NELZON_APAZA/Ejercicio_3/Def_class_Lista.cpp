...
void Lista::Agregar_posicion(Nodo *_nodo_nuevo, int posicion){
    ...
    if (get_cabeza()==NULL){
        ...
        for (int i=0;i<numero_nodos;i++)//avanza toda la lista de nodos
        {
            Nuevo_dato=Avanzar;
            ...
            if (i==posicion)
            {   
                Nuevo_dato->set_enlace(_nodo_nuevo);//poner el nuevo nodo
                Anterior->set_enlace(Avanzar->get_enlace());
            }
        }
