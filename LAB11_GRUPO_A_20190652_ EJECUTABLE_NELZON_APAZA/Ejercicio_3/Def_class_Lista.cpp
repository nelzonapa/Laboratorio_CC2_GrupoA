...
Nodo* Lista::Buscar(int _dato_buscar){
    //para buscar necesitamos ayudarnos de la cabeza, pero no modificarla...
    //para ello, usaremos otro puntero auxiliar llamado BUSQUEDA
    Nodo *Busqueda=get_cabeza();//almacenamos cabeza en buscar
    //Primero si o si debes de poner la condición en caso de no encontrar nada
    while ((Busqueda!=NULL) && (Busqueda->get_numero()!=_dato_buscar))
    {
        Busqueda=Busqueda->get_enlace();//para avnzar al siguiente nodo y seguir buscando
    }
    return Busqueda;//retorna esa dirección
